...
function checkTarget(data, listener) {
   ...
   listener.index = data.index;
   listener.action = data.action;
...
StreamListener.prototype = {
    ...
    onStopRequest: function (aRequest, aContext, aStatus) {
		...
		data.httpStatus = this.aStatus;

        if (components.isSuccessCode(aStatus)) {
            let state = checkSecurity();
		console.log("these are the stored values: " +this.action + " " +this.index);
            this.mCallbackFunc(state, data);
		console.log("callback finished");
        } else {
            this.mCallbackFunc("TARGET_UNDETERMINED", data);
        }
