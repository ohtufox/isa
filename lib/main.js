...
function callbackFunc(state) {
  ...
  // how do I know here which field/address is drawn.
  // pwfield[field-that-belongs-to-callback].drawIcon(state)
  targetState = state;
}
...
function checkTarget(worker) {
	...
	for ( var i = 0; i < elements.length; ++i) {
		...
		if (elements[i].type === 'password') {
			...
			pwFieldMapping[elements[i]] = targetChecker.createListener(callbackFunc); // bind the password field (maybe using an id) to a callback. It creates a new listener but uses the same callbackFunc var. Problem?
