...
function fieldCheck() {
    worker.port.on("fieldcheck", function(request) {
        console.log("let's check action for form (" + request.queue + "): " + request.formAction);

        if (request.formAction != "") {
            console.log('form (' + request.queue + ') formi on!');
            let listener = targetChecker.createListener(new function(state, requestURI) {
                console.log('form (' + request.queue + ') formin tila on ' +state + ', requestURI oli ' + requestURI);
                worker.port.emit(request.queue + 'fieldchecked', state);
            });
            targetChecker.checkTarget(request.formAction, listener);
        } else {
            worker.port.emit(request.queue + 'fieldchecked', 'TARGET_UNDETERMINED');
           // callbackFunc("TARGET_UNDETERMINED", "", request.queue);
        }
    });
}
