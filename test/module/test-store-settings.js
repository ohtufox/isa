...
let script = 'self.port.on("test-send-request", function() {'
            +'self.port.emit("request-storage-status");});'
            +'self.port.on("storage-status", function(status) {self.port.emit("test-requested-status", status);});'
            +'self.port.on("test-change-used", function() {'
            +'self.port.emit("used");});'
            +'';
